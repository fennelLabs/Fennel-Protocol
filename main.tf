...
### BEGIN VALIDATOR 3 NODE ###

resource "google_storage_bucket_object" "fennel-protocol-validator-3-startup" {
  name   = "fennel-protocol-validator-3-terraform-start.sh"
  bucket = "whiteflag-0-admin"
  source = "fennel-protocol-validator-3-terraform-start.sh"
  content_type = "text/plain"
}

resource "google_compute_address" "fennel-protocol-validator-3-ip" {
  name = "fennel-protocol-validator-3-ip"
}

resource "google_compute_instance" "fennel-protocol-validator-3" {
  name         = "fennel-protocol-validator-3-instance"
  machine_type = "e2-standard-2"
  zone         = "us-east1-b"
  ...
  network_interface {
    ...
    subnetwork = "public-subnet"
     access_config {
      nat_ip = google_compute_address.fennel-protocol-validator-3-ip.address
    }
  }

 metadata = {
    startup-script-url = "gs://whiteflag-0-admin/fennel-protocol-validator-3-terraform-start.sh"
    gce-container-declaration = module.gce-container.metadata_value
    google-logging-enabled    = "true"
    google-monitoring-enabled = "true"
  }
...
resource "google_storage_bucket_object" "fennel-protocol-validator-3-ip-address" {
  name   = "fennel-protocol-validator-3-ip.sh"
  bucket = "whiteflag-0-admin"
  content = google_compute_address.fennel-protocol-validator-3-ip.address
}

### END VALIDATOR 3 NODE ###
...
### BEGIN COLLATOR 3 NODE ###

resource "google_storage_bucket_object" "fennel-protocol-collator-3-startup" {
  name   = "fennel-protocol-collator-3-terraform-start.sh"
  bucket = "whiteflag-0-admin"
  source = "fennel-protocol-collator-3-terraform-start.sh"
  content_type = "text/plain"
}

resource "google_compute_address" "fennel-protocol-collator-3-ip" {
  name = "fennel-protocol-collator-3-ip"
}

resource "google_compute_instance" "fennel-protocol-collator-3" {
  name         = "fennel-protocol-collator-3-instance"
  machine_type = "e2-standard-2"
  zone         = "us-east1-b"
  ...
  network_interface {
    ...
    subnetwork = "public-subnet"
     access_config {
      nat_ip = google_compute_address.fennel-protocol-collator-3-ip.address
    }
  }

 metadata = {
    startup-script-url = "gs://whiteflag-0-admin/fennel-protocol-collator-3-terraform-start.sh"
    gce-container-declaration = module.gce-container.metadata_value
    google-logging-enabled    = "true"
    google-monitoring-enabled = "true"
  }
...
resource "google_storage_bucket_object" "fennel-protocol-collator-3-ip-address" {
  name   = "fennel-protocol-collator-3-ip.sh"
  bucket = "whiteflag-0-admin"
  content = google_compute_address.fennel-protocol-collator-3-ip.address
}
