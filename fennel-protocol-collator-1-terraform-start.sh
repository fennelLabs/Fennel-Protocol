...
docker exec fennel-collator-1 /app/target/release/fennel-node key insert --base-path /tmp/parachain/alice --chain raw-parachain-chainspec.json --key-type aura --scheme sr25519 --suri $(gsutil cat gs://whiteflag-0-admin/fennel-collator-1-key.sh)
docker exec fennel-collator-1 /app/target/release/fennel-node --collator --force-authoring --chain raw-parachain-chainspec.json --base-path /tmp/parachain/alice --port 40333 --ws-port 9944 --rpc-port 9934 --telemetry-url "wss://telemetry.polkadot.io/submit/ 0" --rpc-methods Unsafe --name Fennel0 --node-key $(gsutil cat gs://whiteflag-0-admin/fennel-para-key.sh) --unsafe-ws-external --rpc-external --rpc-cors=all -- --execution wasm --chain /app/polkadotspec.json --port 30343 --ws-port 9977 --bootnodes /ip4/$(gsutil cat gs://whiteflag-0-admin/fennel-protocol-boot-ip.sh)/tcp/30333/p2p/$(gsutil cat gs://whiteflag-0-admin/fennel-boot-node-id.sh) --rpc-external --ws-external --prometheus-external --rpc-cors all
