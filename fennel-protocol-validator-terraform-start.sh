...
sudo certbot --nginx --non-interactive --agree-tos --email info@fennellabs.com --domains validator.fennellabs.com
...
docker run fennel-validator-1 /app/target/release/polkadot key insert --base-path /tmp/relay/bob --chain /app/chainspec.json --key-type babe --scheme sr25519 --suri $(gsutil cat gs://whiteflag-0-admin/fennel-validator-key.sh)
docker run fennel-validator-1 /app/target/release/polkadot key insert --base-path /tmp/relay/bob --chain /app/chainspec.json --key-type grandpa --scheme ed25519 --suri $(gsutil cat gs://whiteflag-0-admin/fennel-validator-key.sh)
