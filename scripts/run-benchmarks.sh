...
# This script has two parts which all use the fennel runtime:
...
if [ "$skip_build" != true ]
then
  echo "[+] Compiling fennel-collator benchmarks..."
...
ALL_PALLETS=($(
  $FENNEL_COLLATOR benchmark pallet --list --chain=$chain |\
    tail -n+2 |\
    cut -d',' -f1 |\
    sort |\
    uniq
...
echo "[+] Benchmarking ${#PALLETS[@]} Fennel collator pallets."
...
for PALLET in "${PALLETS[@]}"; do
  ...
  OUTPUT=$(
    $FENNEL_COLLATOR benchmark pallet \
    --chain=$chain \
    --steps=50 \
    --repeat=20 \
    --pallet="$PALLET" \
...
OUTPUT=$(
  $FENNEL_COLLATOR benchmark machine --chain=$chain 2>&1
)
if [ $? -ne 0 ]; then
